...
cmake -Wno-dev -G %CMAKE_GENERATOR% -DCMAKE_INSTALL_PREFIX=%BUILD_DIR% -DPYTHON_LIBRARY=%BUILD_DIR%\lib -DPYTHON_INCLUDE_DIR=%BUILD_DIR%\include -DILMBASE_LOCATION=%BUILD_DIR% -DOPENEXR_LOCATION=%BUILD_DIR% -DWITH_IECORE_GL=1 -DWITH_IECORE_IMAGE=1 -DWITH_IECORE_SCENE=1 %ARNOLD_OPTIONS% -DWITH_IECORE_VDB=1 -DOPENVDB_ROOT=%BUILD_DIR% -DWITH_IECORE_ALEMBIC=1 -DALEMBIC_ROOT=%BUILD_DIR% -DWITH_IECORE_APPLESEED=1 -DAPPLESEED_INCLUDE_DIR=%BUILD_DIR%\appleseed\include -DAPPLESEED_LIBRARY=%BUILD_DIR%\appleseed\lib\appleseed.lib -DOPENVDB_LOCATION=%BUILD_DIR% ..
